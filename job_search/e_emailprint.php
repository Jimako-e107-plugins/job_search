...
where r.jobsch_cid=$id and find_in_set(jobsch_catclass,'" . USERCLASS_LIST . "') and jobsch_approved > 0 and (jobsch_closedate = 0 or jobsch_closedate='' or jobsch_closedate is null or jobsch_closedate>$jobsch_today) ";;
    ...
    if ($pref['jobsch_pictype'] == 1)
    {
        ...
        {
            $jobsch_text .= "<br /><br /><img src='" . e_PLUGIN . "job_search/images/icons/$jobsch_document' style='border:0;' alt='$jobsch_vacancy'/><br />";
        }
    }
    else
    {
        ...
        {
            $jobsch_text .= "<br /><br /><img src='" . $jobsch_document . "' style='border:0;' alt='$jobsch_vacancy'/><br />";
        }
    }
...
<span style=\"font-size: 10px; color: black; font-family: Tahoma, Verdana, Arial, Helvetica; text-decoration: none\">
	...
	<br />
	( http://" . $_SERVER[HTTP_HOST] . e_HTTP . e_PLUGIN . "job_search/jobshack.php?0.item." . $jobsch_cid . ".$jobsch_category.$jobsch_cid )
	</span>";

    require_once(e_HANDLER . 'bbcode_handler.php');
    $this->e_bb = new e_bbcode;
    $jobsch_text = $this->e_bb->parseBBCodes($jobsch_text, '');
    return $jobsch_text;
}
